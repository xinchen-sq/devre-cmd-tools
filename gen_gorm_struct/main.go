...
func main() {
	...
	for rows.Next() {
		...
		//获取单个表所有字段
		cloumn_sql := fmt.Sprintf("select column_name columnName, data_type dataType, column_comment columnComment, column_key columnKey, extra,column_default columnDefault,column_type columnType from information_schema.columns where table_name = '%s' and table_schema = (select database()) order by ordinal_position", table.Name)
		...
		column := Column{}
		for cloumns.Next() {
			err := cloumns.Scan(&column.ColumnName, &column.DataType, &column.ColumnComment, &column.Columnkey, &column.Extra, &column.ColumnDefault, &column.ColumnType)
			//类型判断
			//开始拼接字符串
			if err != nil {
			...
			} else if column.ColumnDefault.String != "" && column.ColumnDefault.String != "CURRENT_TIMESTAMP" {
				struct_str += fmt.Sprintf("`gorm:\"column:%s;default:%v%s\"` \n", column.ColumnName, column.ColumnDefault.String, primaryKey)
			} else {
